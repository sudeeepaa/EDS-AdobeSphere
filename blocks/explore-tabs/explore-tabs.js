import { formatShortDate, truncate, escapeHtml } from '../../scripts/utils.js';
...
function buildEventCard(event) {
  ...
  link.setAttribute('aria-label', event.title || 'Event');

  const imageDiv = document.createElement('div');
  imageDiv.className = 'event-card__image';
  ...
  categoryBadge.textContent = event.category || 'Event';
  ...
  location.textContent = getEventLocationLabel(event);
  body.appendChild(location);

  const excerpt = document.createElement('p');
  excerpt.className = 'event-card__excerpt';
  excerpt.textContent = truncate(event.shortDescription || '', 120);
...
function buildBlogCard(blog) {
  ...
  link.setAttribute('aria-label', blog.title || 'Blog');
  ...
  category.textContent = blog.category || 'Article';
  ...
  excerpt.textContent = truncate(blog.excerpt || '', 140);
  body.appendChild(excerpt);
  ...
  if (author.avatar) {
    ...
    avatar.className = 'blog-card__avatar';
...
function buildCreatorCard(creator) {
  ...
  link.setAttribute('aria-label', creator.name || 'Creator');

  const avatarDiv = document.createElement('div');
  avatarDiv.className = 'creator-card__avatar';
  ...
  stats.className = 'creator-card__stats';

  const creatorStats = creator.stats || {};
  const blogsSpan = document.createElement('span');
  blogsSpan.textContent = `${creatorStats.blogsPublished || 0} Blogs`;
  stats.appendChild(blogsSpan);

  const eventsSpan = document.createElement('span');
  eventsSpan.textContent = `${creatorStats.eventsHosted || 0} Events`;
  stats.appendChild(eventsSpan);
